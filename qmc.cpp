...
void qmc_pla_gen(int* current_interconnect, segment_t* subseg,
  ...
  getinput( subseg);
  init( current_interconnect, and_plane_conf, or_plane_conf,
...
void write_interconnect(unsigned n, unsigned d, int* current_interconnect,
  ...
  for(int i=0;i<arch_segmentBits;i++) {
    or_plane_conf[*current_interconnect*arch_segmentBits+i] = tmp_ci%2;
    tmp_ci >>= 1;
  }
...
void write_pla_config( int* current_interconnect, 
                       char* and_plane_conf, char* or_plane_conf,
                       int arch_selectorBits, int arch_segmentBits){
    ...
    for(j=0;j<final_group[i].size();j++) {
      if(!is_written(final_group[i][j])) {
        write_interconnect(final_group[i][j].number,final_group[i][j].dashes,
                         current_interconnect, and_plane_conf, or_plane_conf,
                         arch_selectorBits, arch_segmentBits);
...
void init( int* current_interconnect,
           char* and_plane_conf, char* or_plane_conf,
           int arch_selectorBits, int arch_segmentBits){
  table.clear();
  p_group.clear();
  ...
  if(show_mid)
    print_p_group();
  create_final_group();
  write_pla_config( current_interconnect,
